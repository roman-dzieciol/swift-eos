    ...
    public func testEOS_Platform_GetModsInterface_Null() throws {
        try autoreleasepool { 
            ...
            __on_EOS_Platform_GetModsInterface = { Handle in
                XCTAssertNil(Handle)
                TestGlobals.current.sdkReceived.append("EOS_Platform_GetModsInterface")
                return nil
            }
            ...
            // Then
            XCTAssertEqual(TestGlobals.current.sdkReceived, ["EOS_Platform_GetModsInterface"])
            XCTAssertNil(result)
        }
        
        // Then
        __on_EOS_Platform_Release = nil
        XCTAssertEqual(TestGlobals.current.sdkReceived, ["EOS_Platform_GetModsInterface", "EOS_Platform_Release"])
    }
}
